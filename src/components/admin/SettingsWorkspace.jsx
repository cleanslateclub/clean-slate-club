...
import { ClipboardCheck, Search, Settings, ShieldAlert, ShieldCheck, SlidersHorizontal, TestTube2, ToggleLeft } from 'lucide-react';
    ...
    <div className="rounded-3xl bg-cream border border-taupe/15 p-4 flex items-start gap-3">
      ...
      <div>
        ...
        <p className="font-body text-sm text-charcoal/45 font-light mt-1 leading-relaxed">
          This view is for checking default rules, saved AppSettings, feature flags, launch guardrails, and smoke-test guidance. It does not update business rules, enable features, unlock automations, or publish launch settings.
        </p>
...
export default function SettingsWorkspace() {
  ...
  const rows = useMemo(() => {
    ...
    if (filter === 'guards' || filter === 'smoke_test') return [];
    return getRuleRows();
  }, [filter, settings]);
    ...
    <div className="space-y-6">
      <div className="bg-warm-white rounded-3xl border border-taupe/15 p-6">
        ...
        <p className="font-body text-sm text-charcoal/45 font-light mt-2 max-w-3xl leading-relaxed">
          Centralizes default rules, saved settings, feature flags, launch guardrails, and smoke-test guidance so the backend OS is not hidden across random files.
        </p>
