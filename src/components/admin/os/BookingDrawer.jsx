  ...
  return (
    <div className="flex items-start gap-2.5 py-1.5 border-b border-taupe/8">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-body text-[10px] uppercase tracking-wider font-bold text-gray-500">{label}</p>
        {link ? (
          <a href={link} target="_blank" rel="noreferrer" className="font-body text-sm text-coral font-semibold hover:underline flex items-center gap-1">
...
function Section({ title, children, defaultOpen = true }) {
  ...
  return (
    <div className="border-b border-taupe/10">
      ...
      >
        <span className="font-body text-xs font-bold uppercase tracking-wider text-gray-600">{title}</span>
...
function ActionButton({ label, icon: Icon, onClick, variant = 'default', locked = false }) {
  const base = 'flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-body font-semibold border transition-all w-full text-left';
  const styles = {
      ...
      >
        ...
        <div className="px-4 py-4 border-b border-taupe/10 bg-cream/50">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="font-heading text-xl font-bold text-gray-900">{booking.client_name}</h2>
              <p className="font-body text-sm font-medium text-gray-600 mt-0.5">
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            {editingNotes ? (
              ...
              >
                <p className="font-body text-sm font-medium text-gray-700">
                  {booking.admin_notes || <span className="italic text-gray-400">Click to add internal note...</span>}
                </p>
              </div>
            )}
          </Section>
          ...
          <Section title="Provider Assignment">
            {editingProvider ? (
              ...
              <div className="flex items-center justify-between">
                <span className="font-body text-sm font-semibold text-gray-800">{booking.provider_name || 'Unassigned'}</span>
          ...
          <Section title="Booking Info" defaultOpen={false}>
            <div className="space-y-1 text-sm font-body text-gray-700 font-medium">
