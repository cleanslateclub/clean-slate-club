...
import { Phone, Mail, MapPin, FileText, Image, MessageSquare, ExternalLink, LockKeyhole } from 'lucide-react';
...
const STATUS_COLORS = {
  ...
  archived: 'bg-cream text-charcoal/45 border-taupe/20',
};

const STATUS_LIST = ['pending', 'confirmed', 'completed', 'cancelled', 'archived'];
const normalize = (value = '') => String(value || '').trim().toLowerCase();

export default function BookingDetail({ booking, onUpdateStatus, updatingId }) {
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      setNoteMessage('Note could not be saved.');
    } finally {
      setSavingNote(false);
    }
  };
    ...
    <div className="bg-warm-white rounded-3xl border border-taupe/15 overflow-hidden">
      <div className="px-6 pt-6 pb-4 border-b border-taupe/10 flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            <span className={`text-[10px] px-2.5 py-0.5 rounded-full border font-body font-light ${STATUS_COLORS[bookingStatus] || STATUS_COLORS.pending}`}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-coral/10 border border-coral/20 text-coral text-xs font-body font-light">
            <LockKeyhole className="w-3 h-3" /> Schedule edits locked
          </span>
      ...
      <div className="p-6 space-y-5">
        <div className="rounded-2xl bg-coral/10 border border-coral/20 p-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-body text-[10px] uppercase tracking-widest text-coral font-light">Direct schedule saves disabled</p>
            <p className="font-body text-xs text-charcoal/45 font-light mt-1 leading-relaxed">
              This legacy detail panel no longer writes schedule edits, TimeBlocks, or Google Calendar changes directly. Use the controlled schedule preview flow until Base44 TimeBlock behavior is verified.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            {STATUS_LIST.map(s => (
              <button
                key={s}
                disabled={bookingStatus === s || updatingId === booking.id}
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-body font-light border transition-all disabled:cursor-default ${
                  bookingStatus === s ? STATUS_COLORS[s] : 'bg-cream border-taupe/20 text-charcoal/50 hover:border-coral/30'
                }`}
              >
                {s}
              </button>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-cream rounded-2xl p-4">
            ...
            <p className="font-body text-xs text-charcoal/45 font-light">
              {booking.scheduled_start_time || 'TBD'} – {booking.scheduled_end_time || 'TBD'}
            </p>
        ...
        <div>
          ...
          <div className="flex items-center justify-between mt-1">
            <p className="font-body text-[10px] text-charcoal/25 font-light">Saves private note only</p>
            {(savingNote || noteMessage) && <p className="font-body text-[10px] text-sage font-light">{savingNote ? 'Saving...' : noteMessage}</p>}
          </div>
        </div>
