...
export default function Step4Schedule({ totalDuration, selectedDate, selectedTime, onSelect }) {
  ...
  return (
    <div>
      ...
      <p className="font-body text-sm text-charcoal/45 font-light mb-2">
      ...
      <p className="font-body text-xs text-charcoal/30 font-light mb-6">Only slots with enough time are shown. We don't double-book.</p>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <button
            ...
            className="font-body text-xs text-charcoal/40 hover:text-coral disabled:opacity-25 transition-colors"
          >← Prev</button>
          <span className="font-body text-xs text-charcoal/35 font-light">
                ...
                className={`flex flex-col items-center py-2.5 px-1 rounded-xl border text-center transition-all duration-200 ${
                  isSelected
                    ...
                    : available
                      ...
                      : 'bg-taupe/10 border-taupe/10 text-charcoal/20 cursor-not-allowed'
                }`}
              >
                ...
                <span className={`font-body text-[9px] font-light mt-0.5 ${isSelected ? 'text-white/70' : 'text-charcoal/35'}`}>
      ...
      {selectedDate && (
        <div>
          <p className="font-body text-xs text-charcoal/40 font-light mb-3">Available start times:</p>
          <div className="flex flex-wrap gap-2">
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-full border text-xs font-body font-light transition-all duration-200 ${
                    selectedTime === time
                      ? 'bg-coral border-coral text-white'
                      : 'bg-warm-white border-taupe/20 text-charcoal/60 hover:border-coral/40'
                  }`}
                >
                  {time} – {endTime}
                </button>
            ...
            {availableSlots.length === 0 && !loading && (
              <p className="font-body text-sm text-charcoal/35 font-light">No available slots this day. Please try another date.</p>
            )}
          </div>
        </div>
      )}
