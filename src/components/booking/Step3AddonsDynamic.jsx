...
import { BUFFER_PREP, BUFFER_WRAP } from '@/lib/bookingConfig';
...
export default function Step3AddonsDynamic({ serviceKey, selectedAddons, onToggle, dynamicEstimate, selectedTasks = [], serviceConfig }) {
  ...
  const allAddons = config.addons || [];
  const availableAddons = allAddons.filter(addon => !isAddOnDisabledByTask(addon.id));
