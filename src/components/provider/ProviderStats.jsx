...
export default function ProviderStats({ bookings, user }) {
  const stats = useMemo(() => {
    const now = new Date();
    ...
    const monthlyRevenue = completed
      ...
      .reduce((sum, b) => sum + (b.estimated_price_low || 0), 0);

    return {
      upcomingThisWeek: thisWeek.filter(b => b.status !== 'completed').length,
      completedThisMonth: completed.filter(b => {
        const bDate = new Date(b.created_date);
        return bDate.getMonth() === now.getMonth() && bDate.getFullYear() === now.getFullYear();
      }).length,
      monthlyRevenue,
  ...
  return (
    <div className="grid grid-cols-3 gap-4 mb-8">
      <div className="bg-gradient-to-br from-coral/5 to-coral/10 rounded-2xl border border-coral/20 p-6">
        <div className="flex items-start justify-between mb-2">
          <div>
            ...
            <p className="font-body text-xs font-light text-charcoal/40 mt-1">Upcoming bookings</p>
          </div>
      ...
      <div className="bg-gradient-to-br from-butter/5 to-butter/10 rounded-2xl border border-butter/20 p-6">
        <div className="flex items-start justify-between mb-2">
          <div>
            <p className="font-body text-xs font-light text-charcoal/50 mb-1">Revenue</p>
            <p className="font-heading text-3xl font-semibold text-charcoal">${Math.round(stats.monthlyRevenue)}</p>
