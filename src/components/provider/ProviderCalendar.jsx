...
export default function ProviderCalendar({
  ...
  const visibleIds = useMemo(() => {
    ...
    return new Set(bookings.filter(b => b.provider_email === filterEmail).map(b => b.id));
  ...
  return (
    <div className="bg-white rounded-3xl border border-taupe/15 shadow-sm p-6 overflow-visible relative">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isAdmin && providers.length > 0 && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-taupe/20 bg-cream">
              ...
              <select value={filterEmail} onChange={e => setFilterEmail(e.target.value)} className="bg-transparent font-body text-sm text-charcoal focus:outline-none cursor-pointer">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => { const d = new Date(selectedWeek); d.setDate(d.getDate() - 7); onWeekChange(d); }} className="p-2 hover:bg-cream rounded-lg transition-colors">
              <ChevronLeft size={20} className="text-charcoal/40" />
            </button>
            <button onClick={() => onWeekChange(new Date())} className="px-3 py-1.5 text-xs font-body text-charcoal/50 hover:bg-cream rounded-lg transition-colors">Today</button>
            <button onClick={() => { const d = new Date(selectedWeek); d.setDate(d.getDate() + 7); onWeekChange(d); }} className="p-2 hover:bg-cream rounded-lg transition-colors">
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className="grid grid-cols-7 gap-1 bg-cream/50 rounded-2xl p-1">
          {weekDates.map((date, dayIdx) => {
            ...
            const dayBlocks = (blocksByDay[dateStr] || []).slice().sort((a, b) => (a.start_time || '').localeCompare(b.start_time || ''));

            return (
              <div key={dateStr} className={`rounded-xl border p-2 min-h-[520px] transition-colors ${isToday ? 'bg-coral/5 border-coral/20' : 'bg-white border-taupe/10'}`}>
                <div className="mb-2 pb-2 border-b border-taupe/10">
                  <p className="font-body text-[10px] font-light text-charcoal/50 uppercase tracking-widest">{DAY_LABELS[dayIdx]}</p>
                            ...
                            {(provided, snapshot) => (
                              <div ref={provided.innerRef} {...provided.draggableProps} {...(isAdmin ? provided.dragHandleProps : {})} className={snapshot.isDragging ? 'opacity-60' : ''} onMouseEnter={e => booking && showHover(e, booking)} onMouseLeave={hideHover}>
                                <CalendarEvent
                                  ...
                                  onStartVisit={onStartVisit && booking ? () => onStartVisit(booking) : null}
                                />
                              </div>
                            )}
                          </Draggable>
      ...
      {hoveredBooking && (() => {
        ...
        const addonLabels = (b.addons || []).map(id => cfg.addons?.find(a => a.id === id)?.label).filter(Boolean);

        const rows = [
          ['📅', 'Date', b.scheduled_date ? new Date(b.scheduled_date + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }) : null],
          ['🕐', 'Time', b.scheduled_start_time ? `${b.scheduled_start_time}${b.scheduled_end_time ? ` – ${b.scheduled_end_time}` : ''}` : null],
          ['🧹', 'Service', cfg.label || b.service_category],
          ['💰', 'Estimate', b.estimated_price_low ? `$${b.estimated_price_low} – $${b.estimated_price_high}` : null],
          ...
          ['📝', 'Visit Notes', b.provider_notes || b.special_notes || ia.special_notes],
          ['📆', 'Booked', b.created_date ? new Date(b.created_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : null],
        ].filter(([,, v]) => v && String(v).trim() && String(v).trim() !== 'undefined');

        return (
          <div style={{ position: 'fixed', left: hoverPos.x, top: hoverPos.y, zIndex: 9999, width: 292 }} className="bg-warm-white border border-taupe/20 rounded-2xl shadow-2xl overflow-hidden" onMouseEnter={keepHover} onMouseLeave={() => setHoveredBooking(null)}>
            <div className="px-4 pt-4 pb-3 border-b border-taupe/10">
              <p className="font-heading text-sm font-semibold text-charcoal leading-tight">{b.client_name}</p>
              <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full border font-body font-light mt-1 ${STATUS_STYLES[b.status] || 'bg-taupe/10 border-taupe/30 text-charcoal/50'}`}>{b.status}</span>
            </div>
            ...
            <div className="px-4 py-3 space-y-1.5 max-h-64 overflow-y-auto">
              {rows.map(([icon, label, value]) => (
                <div key={label} className="flex items-start gap-1.5 text-xs font-body">
                  ...
                  <span className="text-charcoal font-light leading-tight break-words flex-1">{String(value)}</span>
                </div>
              ))}
            </div>
            ...
            {onStartVisit && !['completed', 'cancelled', 'archived'].includes(b.status) && (
              <div className="px-4 pb-4">
                <button onClick={() => { onStartVisit(b); setHoveredBooking(null); }} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-coral text-white font-body text-sm font-bold shadow-lg ring-2 ring-coral/25 hover:bg-coral/85 active:scale-[0.98] transition-all">
