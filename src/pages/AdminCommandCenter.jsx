    ...
    <header className="sticky top-0 z-40 bg-charcoal text-warm-white shadow-sm">
      <div className="h-16 px-5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-[210px]">
          <div className="w-9 h-9 rounded-2xl bg-coral/20 border border-coral/30 flex items-center justify-center font-logo text-coral">C</div>
          <div>
            <p className="font-logo text-xl leading-none">Clean Slate</p>
            <p className="font-body text-[10px] uppercase tracking-[0.24em] text-warm-white/45">Admin OS</p>
          </div>
        </div>
...
function LeftMenu({ selected, onSelect }) {
  return (
    <aside className="w-[320px] shrink-0 h-[calc(100vh-4rem)] overflow-y-auto bg-warm-white border-r border-taupe/15">
            ...
            <div key={section.title}>
              ...
              <div className="space-y-1">
                {section.items.map(item => (
                  <button key={item} type="button" onClick={() => onSelect(section.title, item)} className={`w-full flex items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm font-body ${selected === item ? 'bg-coral/10 text-coral border border-coral/20' : 'text-charcoal/65 hover:bg-cream border border-transparent'}`}>
                    <span>{item}</span><span className="w-2 h-2 rounded-full bg-sage/70" />
                  </button>
                ))}
              </div>
            </div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_350px] gap-4">
      <section className="rounded-3xl bg-warm-white border border-taupe/15 overflow-hidden">
        <div className="px-4 py-3 border-b border-taupe/10 flex items-center justify-between gap-3 flex-wrap">
          <div><p className="font-logo text-3xl text-coral leading-tight">Calendar</p><p className="font-body text-sm text-charcoal/45">Schedule first. Every service, package, appointment, provider and payment routes back here.</p></div>
          <div className="flex gap-2"><button className="rounded-xl bg-coral text-warm-white px-4 py-2 text-xs font-body">Day</button><button className="rounded-xl border border-taupe/20 px-4 py-2 text-xs text-charcoal/55 font-body">Week</button><button className="rounded-xl border border-taupe/20 px-4 py-2 text-xs text-charcoal/55 font-body">Month</button></div>
        </div>
        <div className="grid grid-cols-[72px_1fr] min-h-[560px]">
          <div className="bg-cream/60 border-r border-taupe/10">{['9 AM','10 AM','11 AM','12 PM','1 PM','2 PM','3 PM','4 PM','5 PM'].map(hour => <div key={hour} className="h-[62px] border-b border-taupe/10 px-3 py-2 text-[11px] text-charcoal/35">{hour}</div>)}</div>
          <div className="relative">
            {Array.from({ length: 9 }).map((_, index) => <div key={index} className="h-[62px] border-b border-taupe/10" />)}
            {EVENTS.map((event, index) => <button key={event.title} className={`absolute left-4 right-4 rounded-2xl border p-3 text-left shadow-sm ${event.style}`} style={{ top: 18 + index * 108, minHeight: 82 }}><p className="font-heading text-sm">{event.title}</p><p className="font-body text-[11px] mt-1 opacity-75">{event.time} to {event.end}</p><p className="font-body text-[11px] mt-1 opacity-75">{event.status}</p></button>)}
          </div>
        </div>
      </section>
      <RightDrawer selected={selected} />
    </div>
  );
}
...
function ManageView({ selected }) {
  ...
  return (
    <div className="rounded-3xl bg-warm-white border border-taupe/15 overflow-hidden">
      <div className="px-4 py-3 border-b border-taupe/10 flex items-center justify-between gap-3 flex-wrap"><div><p className="font-logo text-3xl text-coral leading-tight">Manage Services, Packages & Appointments</p><p className="font-body text-sm text-charcoal/45">Create, edit, archive, duplicate, price, schedule, and keep the calendar updated.</p></div><div className="flex gap-2"><button className="rounded-xl border border-taupe/20 px-4 py-2 text-xs text-charcoal/55 font-body">Duplicate</button><button className="rounded-xl border border-coral/30 px-4 py-2 text-xs text-coral font-body">Archive</button><button className="rounded-xl bg-coral px-4 py-2 text-xs text-warm-white font-body">Save Changes</button></div></div>
      <div className="p-4 grid grid-cols-1 xl:grid-cols-[1fr_1fr_320px] gap-4">
        <section className="rounded-2xl border border-taupe/10 bg-cream/30 p-4 space-y-3"><p className="font-body text-[11px] uppercase tracking-[0.18em] text-coral">Details</p><Field label="Name" value={selected} /><Field label="Type" value={data[0]} /><Field label="Visibility" value={data[4]} /></section>
        <section className="rounded-2xl border border-taupe/10 bg-cream/30 p-4 space-y-3"><p className="font-body text-[11px] uppercase tracking-[0.18em] text-coral">Pricing + Schedule</p><Field label="Price" value={data[1]} /><Field label="Duration" value={data[2]} /><Field label="Deposit" value={data[3]} /></section>
        <section className="rounded-2xl border border-dusty-blue/25 bg-dusty-blue/10 p-4"><p className="font-heading text-sm text-charcoal">Updates Everywhere</p>{['Online booking','Packages','Calendar','Provider rules','Checkout','Reports'].map(item => <div key={item} className="mt-2 rounded-xl bg-warm-white border border-taupe/10 px-3 py-2 text-xs text-charcoal/55">{item}</div>)}</section>
      </div>
    </div>
  );
}
