...
export const getBookingOperationsSummary = (bookings = []) => {
  ...
  const noShows = bookings.filter(item => item.status === 'no_show').length;
  const pending = bookings.filter(item => ['pending', 'needs_review', 'approved'].includes(item.status)).length;
  const unassigned = bookings.filter(item => !item.provider_email && !['cancelled', 'archived'].includes(item.status)).length;
  const needsReview = bookings.filter(item => item.requires_admin_approval || item.approval_status === 'pending' || item.status === 'needs_review').length;
...
export const getProviderOperationsSummary = (bookings = []) => {
  ...
  const grouped = groupBy(assigned, item => item.provider_email);
...
export const getMembershipOperationsSummary = (households = []) => ({
  ...
  activeMembers: households.filter(item => item.membership_status === 'active').length,
  pastDueMembers: households.filter(item => item.membership_status === 'past_due').length,
  cancelledMembers: households.filter(item => item.membership_status === 'cancelled').length,
});
